...
class CommittedMetadataTests(unittest.TestCase):
    def test_exact_legacy_v051_and_v0601_metadata(self):
        lock, inventory = locked_inputs(ROOT)
        self.assertEqual((lock['expectedFiles'], lock['expectedBytes']), (1305, 636098834))
        self.assertEqual([r['version'] for r in lock['releases']], ['v0.51.0', 'v0.60.1'])
        legacy = json.loads((ROOT / 'legacy/source-lock.v1.json').read_bytes())
        self.assertEqual(legacy['format'], 'revealline-archive-originals.v1')
        self.assertEqual(legacy['budgetBytes'], LIMIT)
        for key in ('version', 'sourceRevision', 'tagObject', 'metadata', 'distributionSha256', 'distributionBytes'):
            self.assertEqual(lock['releases'][0][key], legacy[key])
        original = json.loads((ROOT / 'legacy/expected-inventory.v1.json').read_bytes())
        rows = [row for row in original['files'] if row['path'].startswith('releases/v0.51.0/')]
        self.assertEqual(len(rows), 618)
        actual = {row['path']: row for row in inventory['files']}
        self.assertEqual([actual[row['path']] for row in rows], rows)
        successor = lock['releases'][1]
        self.assertEqual(successor['tagObject'], '725bbe5e04aba9ee77c4261fb057b9ad54767de1')
        self.assertEqual(successor['sourceRevision'], '822f3290787c704a217e7c185d4b8beb7a527e82')
        self.assertEqual(successor['sourceTree'], '88bd680e09210f9bf56ca18465fdb46afb2f1307')
        self.assertEqual(successor['sourceQualification'], {'bytes': 75071, 'sha256': 'ded582540a39feccf92b83f866ecea71c087abe776a6e0c31d50f789f5d2ebf5'})
        qualifications = [row for row in inventory['files'] if row['path'].endswith('/source-qualification.json')]
        self.assertEqual(len(qualifications), 1)
        self.assertEqual(qualifications[0]['bytes'], 75071)
    ...
    def test_changed_or_wrong_identity_qualification_refuses(self):
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            original = json.loads((ROOT / 'source-lock.json').read_bytes())['releases'][1]
            path = root / 'metadata/v0.60.1/source-qualification.json'
